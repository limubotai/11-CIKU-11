...
module.exports.config = {
	...
	commandCategory: "Box", 
  usePrefix: true,
	usages: "[Reply To Photo You Want To Set As Your Group Image]", 
	cooldowns: 0,
	dependencies: [] 
};
...
module.exports.run = async ({ api, event }) => {
	...
	let pathImg = __dirname + '/tmp/loz.png';
    let getdata = (await axios.get(`${abc}`, { responseType: 'arraybuffer' })).data;
  fs.writeFileSync(pathImg, Buffer.from(getdata, 'utf-8'));
    return api.changeGroupImage(fs.createReadStream(__dirname + '/tmp/loz.png'), event.threadID, () => fs.unlinkSync(pathImg), event.messageID);
}
